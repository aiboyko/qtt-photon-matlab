...
% end
geom{nboxmax+1}=geom_box(params,xb1-depth,xb1-1,yb1,yb2,zb1,zb2);
...
[syz_x,sxz_y,sxy_z,invsxz_y,invsyz_x,invsxy_z]=createUPML(k0,...
    [pml.xlowwidth,pml.xhighwidth,pml.ylowwidth,pml.yhighwidth,pml.zlowwidth,pml.zhighwidth],...
...
F3L_visualize(F3_L,params,'field','Ez','fig',2,'name','Direct solver - Vector Wave','func','abs');
hold on
contour(fullmask1_3D(:,:,2^(dz-1)),2,'color','White');
hold off

vectorwave_refl=abs(F3_L_3d(xtest+1,ytest+1,ztest+1))
...
%         sol.res=norm(AA_EH*sol.x-Ab_EH)/norm(Ab_EH)        
        ...
        'rjacobi','max_full_size',MFS,'nswp',10,...
        'resid_damp',1.5,'local_restart',1 ,'rmax',100,'trunc_norm','fro','kickrank',10,'x0',b);
        time.qtt.solved{1}=cputime;
        wavesol_exp.res{1}=norm(A*wavesol_exp.x{1}-b)/norm(b)
%         for i=2:5
%         wavesol_exp.x{i}=amen_solve2(A,b,tol,'local_prec',...
%              'rjacobi','max_full_size',MFS,'nswp',7,...
%              'resid_damp',2,'local_restart',1 ,'rmax',10*(i+1),'trunc_norm','fro','x0',wavesol_exp.x{i-1});
