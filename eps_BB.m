...
function [lambdas,eps,w] = cef(z,N);

%Visualization Parameters (USER INPUT)
lmin = 300e-9; %Minimum wavelength in visualization
lmax = 600e-9; %Maximum wavelength in visualization
dl = 5e-9; %Visualization wavelength incrament
N = 100000; %Number of iterations when calculating complex error function
...
for lambda = lmin:dl:lmax;
    ...
    eps(count) = epsf + sum(epsb);
    
    %Visualzation over optical wavelengths (compare to Figure 2 of cited article) 
   
end
 figure(2)
    lambdas=lmin:dl:lmax;
    plot(lambdas,real(eps),'-bo')
    hold on
    plot(lambdas,imag(eps),'-xr')    
    hold off
%     axis([lmin lmax ],'auto') %1e-1 1e4
    xlabel('Wavelength [m]')
